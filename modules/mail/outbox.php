...
if ( isset($_GET['delete']) ) {
    ...
    if ( $mail_id !== 0 ) {
        $sql    = "SELECT mail_id FROM mail 
                   WHERE mail_id = " .$mail_id. " AND sender = '" .mysql_real_escape_string($username). "' 
                   LIMIT 1";
        $conn->execute($sql);
...
$sql_count      = "SELECT COUNT(mail_id) AS total_messages FROM mail WHERE sender = '" .mysql_real_escape_string($username). "'
...
$sql            = "SELECT m.*, s.photo, s.gender
				   ...
				   WHERE m.sender = '" .mysql_real_escape_string($username). "'
                   AND m.outbox = '1'
				   AND m.status = '1'

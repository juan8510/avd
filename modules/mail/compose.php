...
if ( isset($_POST['send_mail']) ) {
    ...
    if ( !$errors ) {
        $sql    = "INSERT INTO mail ( sender, receiver, subject, body, inbox, outbox, send_date, status )
                   VALUES ('" .mysql_real_escape_string($username). "', '" .mysql_real_escape_string($receiver). "',
                           '" .mysql_real_escape_string($subject). "', '" .mysql_real_escape_string($body). "',
                           '1', '" .mysql_real_escape_string($compose['save_outbox']). "', '" .date('Y-m-d h:i:s'). "', '1')";
        $conn->execute($sql);
        $messages[] = translate('mail.msg_sent', $receiver);
        if ( $compose['send_self'] ) {
            $sql    = "INSERT INTO mail ( sender, receiver, subject, body, inbox, outbox, send_date, status )
                       VALUES ('" .mysql_real_escape_string($username). "', '" .mysql_real_escape_string($username). "',
                               '" .mysql_real_escape_string($subject). "', '" .mysql_real_escape_string($body). "',

...
if ( isset($_POST['user_id']) && isset($_POST['message']) ) {
    if ( isset($_SESSION['uid']) ) {
        ...
        if ( $uid == $user_id ) {
            $response    = $lang['ajax.invite_friend_self'];
        } else {
            ...
            if ( $status ) {
                if ( $status == 'Confirmed' ) {
                    $response    = $lang['ajax.invite_friend_exists'];
                    ...
                    if ( $auto == '1' ) {
                        ...
                        $response    = $lang['ajax.invite_friend_respond'];
                    }
                }            
            } else {
				if ($auto == '1') {
					...
					$response		= $lang['ajax.invite_friend_sent'];
          		}
			}
        }
    } else {
        $response = $lang['ajax.invite_friend_login'];
    }
}

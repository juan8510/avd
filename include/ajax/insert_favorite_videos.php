...
if ( isset($_POST['page']) ) {
    if ( isset($_SESSION['uid']) ) {
        ...
        $sql            = "SELECT v.VID, v.title FROM video AS v, favourite AS f
                          WHERE f.UID = " .$uid. " AND f.VID = v.VID AND v.embed_code = ''
        ...
        $code[]         = $lang['user.favorites'].' <strong>&middot;</strong>';
        $code[]         = '<a href="#attach_my_videos" id="attach_mcp_my_videos">'.$lang['ajax.my_videos'].'</a>';
        $code[]         = '</div>';
        if ( $videos ) {
            foreach ( $videos as $video ) {
                $code[] = '<div class="video_box_attach">';
                $code[] = '<a href="#attach_favorite_video_' .$video['VID']. '" id="attach_media_video_' .$video['VID']. '"><img src="' .$config['BASE_URL']. '/media/videos/tmb/' .$video['VID']. '/1.jpg" alt="' .htmlspecialchars($video['title'], ENT_QUOTES, 'UTF-8'). '" width="120" /></a>';
                $code[] = '</div>';
            }

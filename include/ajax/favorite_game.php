...
if ( isset($_POST['game_id']) ) {
    ...
    if ( isset($_SESSION['uid']) ) {
        ...
        if ( $conn->Affected_Rows() == 1 ) {
            $data['msg']    = $lang['ajax.favorite_game_exists'];
        } else {
            $sql        = "SELECT GID, type FROM game WHERE GID = " .$game_id. " AND UID = " .$uid. " LIMIT 1";
            $conn->execute($sql);
            if ( $conn->Affected_Rows() == 1 ) {
                $data['msg']    = $lang['ajax.favorite_game_self'];
            } else {
                ...
                $conn->execute($sql);
                $data['msg']    = $lang['ajax.favorite_game_success'];
                $data['status'] = 1;
            }
        }
    } else {
        $data['msg']    = $lang['ajax.favorite_game_login'];
    }
}

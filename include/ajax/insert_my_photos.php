...
if ( isset($_POST['page']) ) {
    if ( isset($_SESSION['uid']) ) {
        ...
        $sql            = "SELECT COUNT(p.PID) AS total_photos FROM photos AS p, albums AS a WHERE a.UID = " .$uid. " AND p.AID = a.AID";
        $rs             = $conn->execute($sql);
        $total          = $rs->fields['total_photos'];
        $pagination     = new Pagination(8, $page);
        $limit          = $pagination->getLimit($total);
        $sql            = "SELECT p.PID, p.caption FROM photos AS p, albums AS a
                           WHERE a.UID = " .$uid. " AND p.AID = a.AID
                           ORDER BY p.PID DESC LIMIT " .$limit;
        $rs             = $conn->execute($sql);
        $photos         = $rs->getrows();
        $page_link      = $pagination->getPagination('media_content', 'p_mc_my_photos_');
        ...
        $code[]         = $lang['ajax.my_photos'].' <strong>&middot;</strong>';
        $code[]         = '<a href="#attach_favorite_photos" id="attach_mcp_favorite_photos">'.$lang['user.favorites'].'</a>';
        $code[]         = '</div>';
        if ( $photos ) {
            $index      = 0;
            ...
            foreach ( $photos as $photo ) {
                ...
                $code[] = '<a href="#attach_my_photo" id="attach_media_photo_' .$photo['PID']. '"><img src="' .$config['BASE_URL']. '/media/photos/tmb/' .$photo['PID']. '.jpg" alt="' .htmlspecialchars($photo['caption'], ENT_QUOTES, 'UTF-8'). '" width="120" /></a>';
                $code[] = '</div>';
                ++$index;
            }            
            $code[]     = '</div>';

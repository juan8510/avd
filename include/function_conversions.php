...
function convert($e,$vinfo,$vid,$vdo_path,$vdoname,$keyint,$lavfopts,$ofps,$mc,$demuxer,$aspect){
	...
	if ($e['action'] != "copy_only"){
		...
		if($width_new > $compare_width || $e['action'] == "encode_ipod") $divby = 16;
		else $divby = 2;
		...
		|*/ case "encode_x264":
			if($config['hd_convert'] == '1'){
				...
				/*
					...
					if(($vinfo['General_FileExtension'] == "flv" || $vinfo['General_FileExtension'] == "mp4") && $vinfo['General_Format'] == "MPEG-4" && $vinfo['Video_Format'] == "AVC" && stristr($vinfo['Video_Title'], "Google Inc.") !== false){
						$copyfile = true;
					}elseif(($vinfo['General_FileExtension'] == "flv" || $vinfo['General_FileExtension'] == "mp4") && $vinfo['General_Format'] == "MPEG-4" && $vinfo['Video_Format'] == "AVC" && $vinfo['General_CodecID'] == "M4V"){
						$copyfile = true;
					}
					if($copyfile){
						if(@copy($src,$hdFile)){

$(document).ready(function(){
    ...
    $("input[id='send_friend_invite']").click(function(event) {
        ...
        function (response) {
            ...
            $("li[id='add_friend']").html('<a href="#invite_friend" id="invition_sent_{$user.UID}">' + lang_friendship + '</a>');
        });
    });
    ...
    $("a[id*='add_block_'],a[id*='remove_block_']").livequery('click', function(event) {
        ...
        function(response) {
            ...
            if ( response.status == 1 ) {
                if ( action == 'add' ) {
                    $("li[id='block_user']").html('<a href="#unblock_user" id="remove_block_' + user_id + '">' + lang_unblock + '</a>');
                } else {
                    $("li[id='block_user']").html('<a href="#block_user" id="add_block_' + user_id + '">' + lang_block + '</a>');
                }
            }
        }, 'json');
    });
    ...
    $("[id*='subscribe_to_']").livequery('click', function(event) {
        ...
        function (response) {
			if (response.status == '1') {
          		$("li[id='handle_subscription']").html('<a href="#unsubscribe_user" id="unsubscribe_from_' + user_id + '">' + lang_unsubscribe + '</a></li>');
			}
   ...
   $("[id*='unsubscribe_from_']").livequery('click', function(event) {
        ...
        function (response) {
            if (response.status == '1') {
          		$("li[id='handle_subscription']").html('<a href="#subscribe_user" id="subscribe_to_' + user_id + '">' + lang_subscribe + '</a>');
      		}

<div id="content">
    <div class="left span-755">
        <div class="box">            
            <div class="bmenu">
                <div class="bmenul" style="width: 57%;"><h2>{$title|upper}</h2></div>
                <div class="bmenur" style="width: 40%;">{literal}
                    ...
                    {/literal}
                    <div id="order_form">
                    <form name="orderUsers" id="orderUsers" method="get" action="{$relative}/users">
            ...
            <div class="bsubmenu">
                <div class="bsubmenul">
                    {if $order == 'mr'}{t c='global.most_recent'}{else}<a href="{url base='users' strip='o' value='mr'}">{t c='global.most_recent'}</a>{/if} <strong>&middot;</strong>
                    {if $order == 'on'}{t c='global.online_now'}{else}<a href="{url base='users' strip='o' value='on'}">{t c='global.online_now'}</a>{/if} <strong>&middot;</strong>
                    {if $order == 'mv'}{t c='global.most_viewed'}{else}<a href="{url base='users' strip='o' value='mv'}">{t c='global.most_viewed'}</a>{/if} <strong>&middot;</strong>
                    {if $order == 'ma'}{t c='global.most_active'}{else}<a href="{url base='users' strip='o' value='ma'}">{t c='global.most_active'}</a>{/if} <strong>&middot;</strong>
                    {if $order == 'tr'}{t c='global.top_rated'}{else}<a href="{url base='users' strip='o' value='tr'}">{t c='global.top_rated'}</a>{/if} <strong>&middot;</strong>
                    {if $order == 'mp'}{t c='global.most_popular'}{else}<a href="{url base='users' strip='o' value='mp'}">{t c='global.most_popular'}</a>{/if}
                </div>
                <div class="bsubmenur">
                    <div id="avatar_form">
            ...
            <div class="blinkp">
                <div class="blinkcl">{t c='global.showing'} <span>{$start_num}</span> {t c='global.to'} <span>{$end_num}</span> {t c='global.of'} <span>{$users_total}</span> {t c='global.users'}.</div>

        ...
        <div class="box">            
            <div class="bmenu">
                ...
                <div class="bmenur">{literal}
                    ...
                    {/literal}
                    <div id="order_form">
                    <form name="orderGames" id="orderGames" method="get" action="{$relative}/search">
            ...
            <div class="bsubmenu">
                <div class="bsubmenul">
                    {if $order == 'bp'}{t c='global.being_played'}{else}<a href="{url base='search' strip='o' value='bp'}">{t c='global.being_played'}</a>{/if} <strong>&middot;</strong>
                    ...
                    {if $order == 'mp'}{t c='global.most_played'}{else}<a href="{url base='search' strip='o' value='mp'}">{t c='global.most_played'}</a>{/if} <strong>&middot;</strong>
            ...
            <div class="blinkp">
                <div class="blinkcl">{t c='global.showing'} <span>{$start_num}</span> {t c='global.to'} <span>{$end_num}</span> {t c='global.of'} <span>{$games_total}</span> {t c='global.games'}.</div>
                <div class="blinkcr"><a href="{$relative}/upload/game">{t c='game.upload.game'}</a></div>
                <div class="clear"></div>
            </div>
            {if $games}
            {section name=i loop=$games}
            <div class="video_box">
                <a href="{$relative}/game/{$games[i].GID}/{$games[i].title|clean}"><img src="{$relative}/media/games/tmb/{$games[i].GID}.jpg" title="{$games[i].title|escape:'html'}" alt="{$games[i].title|escape:'html'}" width="160" height="120" /><br />{if $games[i].type == 'private'}<img alt="" style="position: absolute; left: 0px; top: +15px; width: 160px; height: 120px;" src="{$relative_tpl}/images/{private type='game'}"/>{/if}<span class="font-13 font-bold">{$games[i].title|truncate:20:' ...':true|escape:'html'}</span><br /></a>
                <div class="box_left">

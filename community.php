...
$sql            = "SELECT UID, username, photo, gender FROM signup WHERE account_status = 'Active' AND gender = 'Male'
                   ORDER BY addtime DESC LIMIT 6";
$rs             = $conn->execute($sql);
$male_users     = $rs->getrows();

$sql            = "SELECT UID, username, photo, gender FROM signup WHERE account_status = 'Active' AND gender = 'Female'
                   ORDER BY addtime DESC LIMIT 6";
$rs             = $conn->execute($sql);
$female_users   = $rs->getrows();

$sql            = "SELECT s.UID, s.username, s.photo, s.gender FROM signup AS s, users_online AS u
                   WHERE s.account_status = 'Active' AND u.online > " .(time()-300). "
                   AND s.gender = 'Male' AND s.UID = u.UID
                   ORDER BY s.addtime DESC LIMIT 6";
$rs             = $conn->execute($sql);

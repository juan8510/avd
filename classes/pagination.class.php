...
class Pagination
{
    ...
    function getPagination( $base = NULL, $id=NULL, $index = 3)
    {
        ...
        if ( $this->page != 1 )
            $output[]   = '<li><a href="' .$url . $separator. 'page=' .$prev_page. '"' .$this->getID($prev_page, 'prev_page'). ' class="prevnext">&laquo;'.$lang['global.prev'].'</a></li>';
        if ( $this->total_pages > (($index*2)+3) && $this->page >= ($index+3) ) {
            $output[]   = '<li><a href="' .$url . $separator. 'page=1"' .$this->getID(1). '>1</a></li>';
            $output[]   = '<li><a href="' .$url . $separator. 'page=2"' .$this->getID(2). '>2</a></li>';
        }
        if ( $this->page > $index+3 )
            $output[]   = '&nbsp;...&nbsp;';
        for ( $i=1; $i<=$this->total_pages; $i++ ) {
            if ( $this->page == $i ) {
                $output[]   = '<li><span class="currentpage">' .$this->page. '</span></li>';
            } elseif ( ($i >= ($this->page-$index) && $i < $this->page) or ($i <= ($this->page+$index) && $i > $this->page) ) {
                $output[]   = '<li><a href="' .$url . $separator . 'page=' .$i. '"' .$this->getID($i). '>' .$i. '</a></li>';
            }
        }
        ...
        if ( $this->total_pages > (($index*2)+3) && $this->page <= $this->total_pages-($index+3) ) {
            $output[]   = '<li><a href="' .$url . $separator. 'page=' .($this->total_pages-2). '"' .$this->getID(($this->total_pages-2)). '>' .($this->total_pages-2). '</a></li>';
            $output[]   = '<li><a href="' .$url . $separator. 'page=' .($this->total_pages-1). '"' .$this->getID(($this->total_pages-1)). '>' .($this->total_pages-1). '</a></li>';
        }
        if ( $this->page != $this->total_pages )
            $output[]   = '<li><a href="' .$url . $separator. 'page=' .$next_page. '"' .$this->getID($next_page, 'next_page'). ' class="prevnext">'.$lang['global.next'].'&raquo;</a></li>';
